...
class SyncYomiSyncService(
    ...
    override suspend fun doSync(syncData: SyncData): Backup? {
        try {
            ...
            val finalSyncData = if (remoteData != null) {
                ...
                Logger.d("SyncService") { "Try update remote data with ETag($etag)" }
                mergeSyncData(syncData, remoteData)
            } else {
                // init or overwrite remote data
                Logger.d { "Try overwrite remote data with ETag($etag)" }
                syncData
            }
            ...
            Logger.e { "Error syncing: ${e.message}" }
    ...
    private suspend fun pullSyncData(): Pair<SyncData?, String> {
        ...
        if (response.code == 304) {
            ...
            Logger.i { "Remote server not modified" }
        ...
        if (response.isSuccessful) {
            ...
            return try {
                ...
                Logger.i { "Bad content responded from server" }
                // the body is invalid
    ...
    private suspend fun pushSyncData(syncData: SyncData, eTag: String) {
        ...
        if (response.isSuccessful) {
            ...
            Logger.d { "SyncYomi sync completed" }
        } else if (response.code == 412) {
            // other clients updated remote data, will try next time
            Logger.d { "SyncYomi sync failed with 412" }
        } else {
            val responseBody = response.body.string()
